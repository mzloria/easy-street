...
public class HumanTest
{
    ...
    public void testHumanConstructor()
    {
        ...
        assertEquals("Human direction not initialized correctly!", Direction.NORTH,
                     h.getDirection());
        assertEquals("Human death time not initialized correctly!", 25, h.getDeathTime());
        ...
        for (final Terrain destinationTerrain : Terrain.values())
        {
            ...
            for (final Light currentLightCondition : Light.values())
            {
                if (destinationTerrain == Terrain.GRASS)
                {
                    ...
                    assertTrue("Human should be able to pass GRASS" + ", with light "
        ...
        for (final Terrain t : Terrain.values())
        {
            if (t != Terrain.GRASS && t != Terrain.CROSSWALK)
            {
                ...
                assertEquals("Human chooseDirection() failed "
                             ...
                             human.chooseDirection(neighbors));
            }
    ...
    public void testChooseDirectionOnGrassNearCrosswalk()
    {
        ...
        // toward the crosswalk. Except when that would cause the human to
        // reverse
        // direction. A Human will only reverse direction if no other valid
        // option exits.
        ...
        for (final Direction d : Direction.values())
        {
            ...
            if (d == Direction.EAST)
            {
                assertNotEquals("A human near a crosswalk and facing " + d
                                + " should not reverse direction!", Direction.WEST,
                                human.chooseDirection(neighbors));
            ...
            else
            {
                assertEquals("A human near a crosswalk and facing " + d
                             + " chose a wrong direction!", Direction.WEST,
                             human.chooseDirection(neighbors));
            }
        }
    }
